...
memory_policy() {
    ...
    if [ $mm_policy = "asmm" ]; then
        ...
        MEM_IS_HUGE=$(grep 'MemTotal' /proc/meminfo |awk '{printf ("%d\n",$2*1024-64*1024*1024*1024)}')
        if [ $MEM_IS_HUGE -gt 0 ]; then
            sed -i "s/^INITPARAMS.*$/INITPARAMS=\"memory_target=0,sga_target=${SGA_Bytes}K,pga_aggregate_target=${PGA_Bytes}K,workarea_size_policy=auto,use_large_pages=only,optimizer_index_cost_adj=40,java_jit_enabled=false\"/g" /assets/dbca.rsp
        else
